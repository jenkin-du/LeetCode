...
int main()
{
    int numbers1 = number1Between1AndNMethod1(111);
    int numbers2 = number1Between1AndNMethod2(111);
    if (numbers1 != -1)
    {
...
int number1Between1AndNMethod2(int n)
{
    ...
    while (high > 0)
    {
        ...
        high = n / (weight * 10);
        int low = n % weight;
        int bit = ((n - low) / (weight * 10)) % 10;
        if (bit % 10 > 1)
        {
            numbers += re > 1 ? (high + 1) * weight : high * weight;
        }
        else if (bit % 10 == 1)
        {
